...
describe("Admin Dashboard", () => {
  beforeEach(() => {
    ...
    // Default mocks for initial dashboard bootstrap calls
    fetch.mockImplementation((url) =>
      Promise.resolve({
        ok: true,
        json: async () => {
          if (typeof url === "string" && url.includes("/admin/query-history")) {
            return { query_history: [] };
          }
          if (typeof url === "string" && url.includes("/admin/documents")) {
            return [];
          }
          return {};
        },
  ...
  test("renders login screen when not authenticated", () => {
    ...
    expect(
      screen.getByPlaceholderText("admin@college.edu"),
    ).toBeInTheDocument();
  });
  ...
  test("renders dashboard and navigates to documents tab", () => {
    ...
    const docsTab = screen
      .getAllByText(/Documents/i)
  ...
  test("verifies hover states work correctly for RAG action button", async () => {
    const mockDocs = [
      {
        id: "1",
        filename: "doc1.pdf",
    ...
    await waitFor(() => {
      const ragButton = document.querySelector(
        'button[title="Start RAG Processing"]',
      );
      expect(ragButton).toBeInTheDocument();

      // Verify button has hover classes
      expect(ragButton).toHaveClass("hover:bg-emerald-50");

      // Test hover interaction
      fireEvent.mouseEnter(ragButton);
      fireEvent.mouseLeave(ragButton);
  ...
  test("verifies table responsive behavior", async () => {
    const mockDocs = [
      {
        id: "1",
        filename: "very-long-document-name-that-might-cause-layout-issues.pdf",
        file_type: "application/pdf",
        status: "pending",
        created_at: "2023-12-01T10:00:00Z",
      },
  ...
  test("verifies status badges display correctly for all statuses", async () => {
    ...
    await waitFor(() => {
      ...
      expect(processingBadge).toHaveClass(
        "bg-blue-100",
        "text-blue-700",
        "border-blue-200",
        "animate-pulse",
  ...
  test("verifies pending documents show awaiting approval state", async () => {
    const mockDocs = [
      {
        id: "1",
        filename: "pending_doc.pdf",
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("completed_doc.pdf")).toBeInTheDocument();

      // Pending docs should surface the non-actionable approval state
      const pendingRow = screen.getByText("pending_doc.pdf").closest("tr");
      expect(pendingRow).toHaveTextContent("Awaiting approval");
    });
  });

  test("verifies start RAG action triggers API call and status change", async () => {
    const mockDocs = [
      {
        id: "1",
    ...
    fetch.mockImplementation((url) =>
      Promise.resolve({
        ...
        json: async () => {
          ...
          if (
            typeof url === "string" &&
            url.includes("/admin/trigger-rag-processing")
          ) {
    ...
    // Click start RAG processing button
    const ragButton = document.querySelector(
      'button[title="Start RAG Processing"]',
    );
    ...
    expect(fetch).toHaveBeenCalledWith(
      expect.stringContaining("/admin/trigger-rag-processing"),
  ...
  test("verifies document upload functionality is preserved", async () => {
    ...
    expect(
      screen.getByText(/Drag and drop your syllabus, placement PDFs/),
    ).toBeInTheDocument();
  });
});
