...
try:
    ...
    logger.info(
        f"Configuration validated successfully: {system_config.application.app_name} v{system_config.application.app_version}"
    )
...
async def _trigger_rag_processing_with_status_tracking(
    document_id: str, filename: str, approved_by: str
):
    """
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Failed to update document status to processing: {str(status_error)}"
            )
        ...
        logger.info(
            f"RAG processing completed successfully for document {document_id} ({filename})"
        )

    except Exception as e:
        logger.error(
            f"RAG processing failed for document {document_id} ({filename}): {str(e)}"
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to update document status to failed: {str(status_error)}"
            )
        ...
        try:
            ...
            logger.warning(
                f"Failed to create processing failure notification: {str(notification_error)}"
            )
...
async def trigger_manual_rag_processing(
    ...
    if current_user["role"] != "college_admin":
        raise HTTPException(
            status_code=403, detail="Not authorized to trigger RAG processing"
        )
    ...
    try:
        ...
        logger.info(
            f"Manual RAG processing triggered for document {document_id} ({filename}) by user {current_user['user_id']}"
        )
        ...
        logger.error(
            f"Failed to trigger manual RAG processing for document {document_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500, detail=f"Failed to trigger RAG processing: {str(e)}"
        )
    ...
    try:
        ...
        try:
            ...
            if (
                ...
                or "email_already_in_use" in msg
...
async def list_public_colleges():
    ...
    try:
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Failed to load colleges: {str(e)}"
        )
...
async def create_chat(
    ...
    create_chat.rate_limit_cache = {
        uid: timestamps
        for uid, timestamps in create_chat.rate_limit_cache.items()
    ...
    logger.info(
        f"Processing chat message from user {user_id_str} in conversation {message.conversation_id}"
    )
    ...
    try:
        ...
        try:
            ...
            if current_user["user_id"] != str(message.user_id):
                logger.warning(
                    f"User {current_user['user_id']} attempted to send message as user {message.user_id}"
                )
                raise HTTPException(
                    status_code=403,
        ...
        try:
            ...
            if not conv_check.data:
                ...
                logger.info(
                    f"Created new conversation {message.conversation_id} for user {message.user_id}"
                )
            else:
                ...
                if existing_conv["user_id"] != str(message.user_id):
                    logger.warning(
                        f"User {message.user_id} attempted to access conversation {message.conversation_id} owned by {existing_conv['user_id']}"
                    )
                    raise HTTPException(
                        status_code=403,
                ...
                if existing_conv["college_id"] != college_id:
                    logger.warning(
                        f"College ID mismatch for conversation {message.conversation_id}"
                    )
                    raise HTTPException(
            ...
            logger.error(
                f"Error managing conversation {message.conversation_id}: {str(e)}"
            )
        ...
        try:
            ...
            logger.debug(
                f"Stored user message in conversation {message.conversation_id}"
            )
        ...
        try:
            ...
            logger.info(
                f"Attempting RAG response for query: '{message.content[:100]}...' (college: {college_id})"
            )
            ...
            try:
                ...
                if history_response.data:
                    ...
                    logger.debug(
                        f"Retrieved {len(conversation_history)} messages for conversation context"
                    )

            except Exception as history_error:
                logger.warning(
                    f"Failed to retrieve conversation history: {str(history_error)}"
                )
            ...
            if rag_result.get("fallback_used", False):
                ...
                logger.warning(
                    f"RAG fallback used for query: {message.content[:50]}..."
                )
            else:
                quality_score = rag_result.get("quality_score", 0.0)
                logger.info(
                    f"RAG response generated successfully using {rag_result.get('chunks_used', 0)} chunks from {len(rag_result.get('sources', []))} sources (quality: {quality_score:.2f})"
                )

        except (ImportError, ModuleNotFoundError) as import_error:
            logger.warning(f"RAG not available, using basic chat: {str(import_error)}")
            rag_result = await generate_basic_response(
                query=message.content, college_id=college_id
            )
            response_metadata["fallback_used"] = True
            ...
            if (
                ...
                and isinstance(e, EmbeddingServiceError)
            ):
                logger.error(
                    f"Embedding service error for user {message.user_id}: {str(e)}"
                )
                ...
                try:
                    ...
                    response_metadata["fallback_used"] = True
                    logger.info(
                        f"Fallback to basic chat successful for user {message.user_id}"
                    )
                ...
                try:
                    ...
                    response_metadata["fallback_used"] = True
                    logger.info(
                        f"Fallback to basic chat successful after vector store error"
                    )
                except Exception as fallback_error:
                    logger.error(f"Basic chat fallback failed: {str(fallback_error)}")
                    raise HTTPException(
                ...
                logger.error(
                    f"Unexpected error during RAG processing for user {message.user_id}: {str(e)}"
                )
                ...
                try:
                    ...
                    logger.error(
                        f"All fallback mechanisms failed: {str(fallback_error)}"
                    )
                    raise HTTPException(
                        status_code=503,
        ...
        try:
            ...
            quality_score = rag_result.get("quality_score", 0.0)
            conversation_context_used = rag_result.get(
            ...
            for i, attempt_data in enumerate(attempts):
                try:
                    ...
                    if assistant_message_response.data:
                        if i > 0:
                            logger.warning(
                                f"Message stored using fallback attempt {i + 1} (some columns may be missing from database)"
                            )
                        break
                    ...
                    if i < len(attempts) - 1:  # Not the last attempt
                        ...
                        if (
                            ...
                            logger.debug(
                                f"Attempt {i + 1} failed due to missing column, trying next approach"
                            )
                            continue
            ...
            if not assistant_message_response or not assistant_message_response.data:
                raise Exception(
                    f"Failed to store assistant message after all attempts. Last error: {str(last_error)}"
                )

            logger.info(
                f"Stored assistant response in conversation {message.conversation_id} (sources: {len(sources_data)}, chunks: {chunks_used}, quality: {quality_score:.2f})"
            )
        ...
        logger.info(
            f"Chat response completed for user {message.user_id}: {response_data['metadata']['response_type']} response with {chunks_used} chunks (quality: {quality_score:.2f})"
        )
        ...
        logger.error(
            f"Unexpected error processing chat message for user {message.user_id}: {str(e)}"
        )
        import traceback
...
async def guest_chat(request: GuestChatRequest):
    ...
    try:
        ...
        try:
            ...
            rag_result = await generate_rag_response(
                query=request.content, college_id=college_id, conversation_history=[]
            )
        except Exception as rag_error:
            logger.warning(
                f"Guest chat RAG failed, falling back to basic response: {str(rag_error)}"
            )
            rag_result = await generate_basic_response(
...
async def upload_document(
    ...
    try:
        ...
        existing_docs = (
            ...
            .in_("status", ["pending_approval", "approved", "processing", "completed"])
            .execute()
        )
        ...
        if existing_docs.data:
            import httpx
            from app.core.database import service_key
            from app.core.database import url as supabase_url
                    ...
                    try:
                        async with httpx.AsyncClient(trust_env=False) as http_client:
                            check_response = await http_client.head(
                                check_url, headers=check_headers, timeout=10.0
                            )
            ...
            try:
                async with httpx.AsyncClient(trust_env=False) as http_client:
                    r = await http_client.post(
                        storage_url, content=file_content, headers=headers, timeout=60.0
                    )
                ...
                if (
                    r.status_code in retryable_status_codes
                ...
                raise HTTPException(
                    status_code=400, detail=f"File upload failed: {r.text}"
                )
        ...
        if not upload_succeeded:
            logger.error(
                f"Storage upload failed after retries for path {path}: {last_upload_error}"
            )
            raise HTTPException(
                status_code=502,
        ...
        async with httpx.AsyncClient(trust_env=False) as http_client:
            r_db = await http_client.post(
                db_url, json=doc_data, headers=db_headers, timeout=10.0
            )
            if r_db.status_code not in [200, 201]:
                raise HTTPException(
                    status_code=500, detail=f"Database error: {r_db.text}"
                )
                ...
                try:
                    ...
                    async with httpx.AsyncClient(trust_env=False) as http_client:
                        await http_client.delete(
                            delete_url, headers=headers, timeout=10.0
                        )
        ...
        try:
            ...
            client = get_service_client()
            super_admin_query = (
                client.table("profiles").select("id").eq("role", "super_admin")
            )
...
async def get_query_history(
    ...
    if current_user["role"] != "college_admin":
        raise HTTPException(
            status_code=403, detail="Not authorized to view query history"
        )
...
async def get_documents(
    ...
    try:
        ...
        query = (
            client.table("documents").select("*").eq("college_id", target_college_id)
        )
        ...
        for doc in documents:
            ...
            if doc.get("status") == "completed":
                try:
                    ...
                    logger.warning(
                        f"Failed to get chunk count for document {doc['id']}: {str(chunk_error)}"
                    )
        ...
        try:
            rag_ready_query = client.rpc(
                "get_vector_storage_stats", {"target_college_id": target_college_id}
            )
        ...
        college_query = (
            client.table("colleges").select("name").eq("id", target_college_id)
        )
        ...
        profile_query = (
            client.table("profiles").select("*").eq("id", current_user["user_id"])
        )
        ...
        return {
            ...
            "college_info": {"id": target_college_id, "name": college_name},
            "user_profile": {
            ...
            "filters": {"status": status, "sort_by": sort_by, "sort_order": sort_order},
        }
...
async def get_pending_documents(current_user: dict = Depends(get_current_user)):
    if current_user["role"] != "super_admin":
        raise HTTPException(
            status_code=403, detail="Not authorized to view pending documents"
        )
    ...
    try:
        ...
        for doc in response.data:
            # Get college name
            college_query = (
                client.table("colleges").select("name").eq("id", doc["college_id"])
            )
            college_response = college_query.execute()
            college_name = (
                college_response.data[0]["name"]
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve pending documents: {str(e)}"
        )
...
async def approve_document(
    ...
    if current_user["role"] != "super_admin":
        raise HTTPException(
            status_code=403, detail="Not authorized to approve documents"
        )
    ...
    try:
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                async with httpx.AsyncClient(trust_env=False) as http_client:
                    r_db = await http_client.patch(
                        db_url, json=update_data, headers=db_headers, timeout=20.0
                    )
                ...
                if (
                    r_db.status_code in retryable_status_codes
                ...
                raise HTTPException(
                    status_code=500, detail=f"Database update error: {r_db.text}"
                )
                ...
                if attempt < max_attempts:
                    ...
                    continue
                logger.error(
                    f"Approve document DB connection failed after retries: {db_error}"
                )
                raise HTTPException(
                    status_code=502,
        ...
        try:
            async with httpx.AsyncClient(trust_env=False) as http_client:
                r_approval = await http_client.post(
                    approval_url, json=approval_data, headers=db_headers, timeout=20.0
                )
                if r_approval.status_code not in [200, 201]:
                    # Log but don't fail the approval if approval record creation fails
                    print(
                        f"Warning: Failed to create approval record: {r_approval.text}"
                    )
        ...
        if request.process_schedule == "immediate":
            ...
            logger.info(
                f"Triggering immediate RAG processing for document {request.document_id} ({document['filename']})"
            )
...
async def reject_document(
    request: DocumentRejectionRequest, current_user: dict = Depends(get_current_user)
):
    if current_user["role"] != "super_admin":
        raise HTTPException(
            status_code=403, detail="Not authorized to reject documents"
        )
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if attempt < max_attempts:
                    ...
                    continue
                logger.error(
                    f"Reject document DB connection failed after retries: {db_error}"
                )
                raise HTTPException(
                    status_code=502,
        ...
        try:
            async with httpx.AsyncClient(trust_env=False) as http_client:
                ...
                if r_approval.status_code not in [200, 201]:
                    ...
                    print(
                        f"Warning: Failed to create rejection record: {r_approval.text}"
                    )
        except Exception as rejection_record_error:
            print(
                f"Warning: Failed to create rejection record: {rejection_record_error}"
            )
...
async def schedule_document_processing(
    request: ScheduleProcessingRequest, current_user: dict = Depends(get_current_user)
):
    """Schedule document processing for a future time."""
    if current_user["role"] != "super_admin":
        raise HTTPException(
            status_code=403, detail="Not authorized to schedule processing"
        )
    ...
    try:
        ...
        if document["status"] != "approved":
            raise HTTPException(
                status_code=400,
                detail=f"Document must be approved to schedule processing. Current status: {document['status']}",
            )
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if attempt < max_attempts:
                    ...
                    continue
                logger.error(
                    f"Schedule processing DB connection failed after retries: {db_error}"
                )
                raise HTTPException(
                    status_code=502,
...
async def trigger_processing(
    ...
    if current_user["role"] != "super_admin":
        raise HTTPException(
            status_code=403, detail="Not authorized to trigger processing"
        )
    ...
    try:
        ...
        async with httpx.AsyncClient() as http_client:
            r_db = await http_client.patch(
                db_url, json=update_data, headers=db_headers, timeout=10.0
            )
            if r_db.status_code not in [200, 201]:
                raise HTTPException(
                    status_code=500, detail=f"Database update error: {r_db.text}"
...
async def get_scheduled_documents(current_user: dict = Depends(get_current_user)):
    ...
    if current_user["role"] != "super_admin":
        raise HTTPException(
            status_code=403, detail="Not authorized to view scheduled documents"
        )
    ...
    try:
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve scheduled documents: {str(e)}"
        )
...
async def get_superadmin_stats(current_user: dict = Depends(get_current_user)):
    ...
    if current_user["role"] != "super_admin":
        raise HTTPException(
            status_code=403, detail="Not authorized to view superadmin stats"
        )
    ...
    try:
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve superadmin stats: {str(e)}"
        )
...
async def get_superadmin_colleges(
    ...
    try:
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve colleges: {str(e)}"
        )
...
async def get_superadmin_admins(
    ...
    try:
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve admins: {str(e)}"
        )
...
async def create_superadmin_admin(
    ...
    try:
        ...
        try:
            ...
            user = getattr(auth_resp, "user", None) or getattr(
                auth_resp, "data", {}
            ).get("user")
            if not user:
                raise HTTPException(
        ...
        try:
            ...
            client.table("admins").upsert(
                admin_row, on_conflict="user_id,college_id"
            ).execute()
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Failed to create admin record: {str(admin_err)}",
            )
...
async def toggle_superadmin_admin_status(
    ...
    if current_user["role"] != "super_admin":
        raise HTTPException(
            status_code=403, detail="Not authorized to update admin status"
        )
    ...
    try:
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Failed to update admin status: {str(e)}"
        )
...
async def get_superadmin_documents(
    ...
    try:
        ...
        if search:
            ...
            for g in groups:
                if (
                    lowered in g["college"].lower()
                    or lowered in g["admin_name"].lower()
                ):
                    filtered_groups.append(g)
                    continue

                # Check documents inside the group
                docs_match = [d for d in g["documents"] if lowered in d["name"].lower()]
                if docs_match:
                    g_copy = g.copy()
...
async def get_notifications(
    ...
    try:
        ...
        notifications = await notification_manager.get_notifications(
            user_id=UUID(current_user["user_id"]), filters=filters
        )
...
async def mark_notification_read(
    ...
    try:
        success = await notification_manager.mark_as_read(
            notification_id=notification_id, user_id=UUID(current_user["user_id"])
        )
