...
class ServiceHealthManager:
    ...
    def record_failure(self, service_name: str, error: Exception) -> None:
        ...
        if health.failure_count >= self._failure_threshold:
            ...
            health.circuit_breaker_open_until = datetime.utcnow() + timedelta(
            ...
            logger.warning(
                f"Service {service_name} failure {health.failure_count}/{self._failure_threshold}: {str(error)}"
            )
    ...
    def is_circuit_breaker_open(self, service_name: str) -> bool:
        ...
        if (
            ...
            and datetime.utcnow() > health.circuit_breaker_open_until
        ):
            ...
            health.failure_count = 0
            logger.info(
                f"Circuit breaker HALF-OPEN for service {service_name} - allowing test requests"
            )
            return False
    ...
    def get_all_service_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        for service_name, health in self._services.items():
            status[service_name] = {
                ...
                "circuit_breaker_open_until": health.circuit_breaker_open_until.isoformat()
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            factor = (
                ...
                else ai_config.retry_backoff_factor
            )
            max_delay_limit = (
                max_delay if max_delay is not None else ai_config.max_retry_delay
            )
            ...
            if service_name and health_manager.is_circuit_breaker_open(service_name):
                logger.error(
                    f"Circuit breaker is OPEN for service {service_name} - failing fast"
                )
            ...
            for attempt in range(retries + 1):
                try:
                    ...
                    if attempt > 0:
                        logger.info(
                            f"Function {func.__name__} succeeded on attempt {attempt + 1}/{retries + 1}"
                        )
                    ...
                    if "quota" in str(e).lower() or "rate" in str(e).lower():
                        ...
                        logger.warning(
                            f"Rate limit detected in {func.__name__}, attempt {attempt + 1}/{retries + 1}. Waiting {rate_limit_delay}s before retry..."
                        )
                        if attempt < retries:
                    ...
                    if attempt == retries:
                        logger.error(
                            f"Function {func.__name__} failed after {retries + 1} attempts: {str(e)}"
                        )
                        ...
                        if logger.isEnabledFor(logging.DEBUG):
                            logger.debug(
                                f"Stack trace for {func.__name__} failure:\n{traceback.format_exc()}"
                            )

                        raise e

                    logger.warning(
                        f"Function {func.__name__} failed on attempt {attempt + 1}/{retries + 1}: {str(e)}. Retrying in {current_delay}s..."
                    )
    ...
    try:
        ...
        if not text.strip():
            raise DocumentProcessingError(
                f"No text could be extracted from PDF ({len(reader.pages)} pages processed)"
            )

        logger.info(
            f"Successfully extracted text from {pages_processed}/{len(reader.pages)} pages"
        )
        return text.strip()
...
def generate_embedding(text: str) -> list[float]:
    ...
    if (
        not ai_config.gemini_api_key
        or ai_config.gemini_api_key == "REPLACE_WITH_YOUR_VALID_GEMINI_API_KEY"
    ):
        logger.warning(
            "GEMINI_API_KEY not configured or using placeholder - returning zero embedding"
        )
    ...
    try:
        ...
        if len(embedding) != TARGET_EMBEDDING_DIMENSION:
            ...
            if original_dim > TARGET_EMBEDDING_DIMENSION:
                ...
                embedding = embedding + (
                    [0.0] * (TARGET_EMBEDDING_DIMENSION - original_dim)
                )
                logger.warning(
                    f"Embedding dimension {original_dim} does not match target {TARGET_EMBEDDING_DIMENSION}; "
        ...
        logger.debug(
            f"Generated embedding of dimension {len(embedding)} for text of length {len(text)}"
        )
        return embedding
...
async def process_document(document_id: str, file_path: str):
    ...
    logger.info(
        f"Processing configuration - chunk_size: {rag_config.chunk_size}, chunk_overlap: {rag_config.chunk_overlap}"
    )
    ...
    try:
        ...
        try:
            doc_res = (
                ...
                .select("college_id, status, filename, uploaded_by")
                .eq("id", document_id)
                .execute()
            )
            if not doc_res.data:
            ...
            logger.info(
                f"Document details retrieved: {filename} (college: {college_id})"
            )
        ...
        while download_attempts < max_download_attempts:
            try:
                ...
                if download_attempts >= max_download_attempts:
                    logger.error(
                        f"All download attempts failed for document {document_id}"
                    )
                    raise DocumentProcessingError(
                        f"Failed to download file from storage after {max_download_attempts} attempts: {str(e)}"
                    )
        ...
        try:
            text_chunks = chunk_text(
                text, rag_config.chunk_size, rag_config.chunk_overlap
            )
        ...
        for batch_start in range(0, len(text_chunks), batch_size):
            ...
            logger.info(
                f"Processing chunk batch {batch_start // batch_size + 1}/{(len(text_chunks) + batch_size - 1) // batch_size} ({len(batch_chunks)} chunks)"
            )
            ...
            for chunk_content in batch_chunks:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to generate embedding for chunk {chunk_counter}: {str(e)}"
                    )

                    # Enhanced failure handling with different strategies based on error type
                    if isinstance(e, RateLimitError):
                        logger.warning(
                            f"Rate limit encountered, waiting before continuing..."
                        )
                        await asyncio.sleep(30)  # Wait 30 seconds for rate limit
                    elif isinstance(e, CircuitBreakerError):
                        logger.error(
                            f"Circuit breaker is open for embedding service, stopping processing"
                        )
                        break

                    # If too many embedding failures, fail the entire process
                    failure_rate = (
                        embedding_failures / (embedding_failures + embedding_successes)
                        if (embedding_failures + embedding_successes) > 0
                        else 1.0
                    )
                    if (
                        failure_rate > 0.5 and embedding_failures > 5
                    ):  # More than 50% failures and at least 5 failures
                    ...
                    embedding_failures += 1
                    logger.error(
                        f"Unexpected error generating embedding for chunk {chunk_counter}: {str(e)}"
                    )
        ...
        logger.info(
            f"Successfully processed {len(chunks_to_insert)} chunks with {embedding_failures} failures"
        )
        processing_stats["stages_completed"].append("embeddings_generated")

        logger.info(f"Stage 6.5: Clearing existing chunks for document {document_id}")
        try:
            client.table("document_chunks").delete().eq(
                "document_id", document_id
            ).execute()
            processing_stats["stages_completed"].append("existing_chunks_cleared")
        except Exception as e:
            error_msg = f"Failed to clear existing chunks: {str(e)}"
            logger.error(error_msg)
        ...
        try:
            ...
            for batch_start in range(0, len(chunks_to_insert), insert_batch_size):
                ...
                try:
                    ...
                    logger.debug(
                        f"Inserted batch {batch_start // insert_batch_size + 1}: {len(batch_data)} chunks"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to insert chunk batch {batch_start // insert_batch_size + 1}: {str(e)}"
                    )
                    raise VectorStoreError(
                        f"Failed to store chunk batch in database: {str(e)}"
                    )

            logger.info(
                f"Successfully stored {total_inserted} chunks in database for document {filename}"
            )
        ...
        try:
            ...
            logger.warning(
                f"Failed to create processing completion notification: {str(e)}"
            )

        # Final success logging
        logger.info(
            f"Successfully completed processing document {document_id} ({filename})"
        )
        logger.info(
            f"Processing summary: {len(chunks_to_insert)} chunks created, {embedding_failures} embedding failures, {processing_stats['total_processing_time']:.2f}s total time"
        )
        ...
        try:
            ...
            if doc_res.data and doc_res.data[0]["status"] == "processing":
                ...
                logger.error(
                    f"Document {document_id} marked as failed with processing stats: {processing_stats}"
                )
            ...
            logger.error(
                f"Error during cleanup for document {document_id}: {str(cleanup_error)}"
            )
...
async def trigger_rag_processing(document_id: str):
    ...
    try:
        ...
        doc_res = (
            client.table("documents")
            .select("storage_path, status, filename")
            .eq("id", document_id)
            .execute()
        )
        if not doc_res.data:
        ...
        if not document.get("storage_path"):
            raise DocumentProcessingError(
                f"Document {document_id} ({filename}) has no storage path"
            )

        logger.info(
            f"Triggering RAG processing for document {document_id} ({filename})"
        )
        ...
        logger.error(
            f"Error triggering RAG processing for document {document_id}: {error_msg}"
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to update document status after trigger failure: {str(update_error)}"
            )
...
async def _check_vector_storage_integrity(client, college_id: str) -> None:
    ...
    try:
        ...
        if chunks_without_embeddings.data:
            logger.warning(
                f"Found {len(chunks_without_embeddings.data)} chunks without embeddings for college {college_id}"
            )
        ...
        if orphaned_chunks.data:
            ...
            valid_docs = (
                client.table("documents").select("id").in_("id", document_ids).execute()
            )
            valid_doc_ids = {doc["id"] for doc in valid_docs.data}

            orphaned_count = sum(
                1
                for chunk in orphaned_chunks.data
                if chunk["document_id"] not in valid_doc_ids
            )

            if orphaned_count > 0:
                logger.warning(
                    f"Found {orphaned_count} orphaned chunks for college {college_id}"
                )
        ...
        if chunks_with_embeddings.data:
            ...
            for chunk in chunks_with_embeddings.data:
                if chunk.get("embedding") and len(chunk["embedding"]) != expected_dim:
                    raise VectorStoreError(
                        f"Embedding dimension mismatch: expected {expected_dim}, got {len(chunk['embedding'])}"
                    )
...
async def _retrieve_chunks_with_python_search(
    ...
    try:
        all_chunks_response = (
            client.table("document_chunks")
            .select("id, document_id, college_id, content, embedding, metadata")
            .eq("college_id", college_id)
            .not_.is_("embedding", "null")
            .execute()
        )
    ...
    try:
        document_ids = list(
            set([chunk["document_id"] for chunk in all_chunks_response.data])
        )
        docs_response = (
            client.table("documents")
    ...
    for chunk in all_chunks_response.data:
        try:
            ...
            processing_errors += 1
            logger.warning(
                f"Error processing chunk {chunk.get('id', 'unknown')}: {str(e)}"
            )
            continue

    if processing_errors > 0:
        logger.warning(
            f"Encountered {processing_errors} errors while processing chunks"
        )
...
async def retrieve_relevant_chunks(
    query: str, college_id: str, top_k: int = None, similarity_threshold: float = None
) -> List[dict]:
    """
    ...
    try:
        ...
        similarity_threshold = max(
            0.0, min(similarity_threshold, 1.0)
        )  # Limit between 0-1
        ...
        try:
            ...
            if result_chunks is not None:
                logger.info(
                    f"Retrieved {len(result_chunks)} chunks using native vector search"
                )
                return result_chunks
        except Exception as e:
            logger.warning(
                f"Native vector search failed, falling back to Python implementation: {str(e)}"
            )
        ...
        logger.info(
            f"Retrieved {len(result_chunks)} relevant chunks for query: '{query[:50]}...'"
        )
        return result_chunks
...
async def generate_rag_response(
    ...
    logger.info(
        f"Generating RAG response for query: '{query[:100]}...' (college: {college_id})"
    )
    ...
    if (
        ...
        logger.warning(
            "GEMINI_API_KEY not configured or using placeholder - falling back to basic response"
        )
    ...
    if health_manager.is_circuit_breaker_open("gemini_embedding"):
        logger.warning(
            "Embedding service circuit breaker is open - falling back to basic response"
        )
        from app.core.basic_chat import generate_basic_response
    ...
    if health_manager.is_circuit_breaker_open("gemini_generation"):
        logger.warning(
            "Generation service circuit breaker is open - falling back to basic response"
        )
        from app.core.basic_chat import generate_basic_response
    ...
    try:
        ...
        if context_chunks is None:
            try:
                ...
                logger.error(
                    f"Unexpected chunk retrieval error details: {traceback.format_exc()}"
                )
                ...
                basic_result["fallback_reason"] = (
                    f"Unexpected chunk retrieval error: {str(e)}"
                )
                basic_result["error_details"] = str(e)
                return basic_result
        ...
        try:
            ...
            for i, chunk in enumerate(context_chunks):
                if not chunk.get("filename") or not chunk.get("content"):
                    logger.warning(
                        f"Skipping chunk {i} due to missing filename or content"
                    )
                    continue
            ...
            logger.info(
                f"Context formatted successfully: {len(context_text)} characters"
            )
        ...
        sources = list(
            set(
                [chunk["filename"] for chunk in context_chunks if chunk.get("filename")]
            )
        )
...
"""
                    logger.info(
                        f"Conversation context built: {len(conversation_parts)} messages"
                    )
...
**Your Answer (cite sources when referencing specific information):**"""
        ...
        try:
            ...
            if "quota" in error_str or "rate" in error_str:
                logger.warning(
                    "Rate limit detected in AI generation - falling back to basic response"
                )
                from app.core.basic_chat import generate_basic_response
                ...
                logger.error(
                    "AI service authentication error - falling back to basic response"
                )
                from app.core.basic_chat import generate_basic_response
                ...
                basic_result["fallback_used"] = True
                basic_result["fallback_reason"] = (
                    f"AI service authentication error: {str(e)}"
                )
                basic_result["error_details"] = str(e)
                return basic_result
            else:
                logger.error(
                    "General AI service error - falling back to basic response"
                )
                from app.core.basic_chat import generate_basic_response
        ...
        try:
            quality_score = _validate_response_quality(
                response_text, context_chunks, query
            )
        ...
        logger.info(
            f"RAG response generated successfully in {processing_time:.2f}s using {len(context_chunks)} chunks from {len(sources)} sources (quality: {quality_score:.2f})"
        )
...
def _validate_response_quality(
    response_text: str, context_chunks: List[dict], query: str
) -> float:
    """
    ...
    try:
        ...
        citations_found = sum(
            1 for source in source_names if source.lower() in response_text.lower()
        )
        ...
        structure_indicators = [
            ...
            any(
                marker in response_text for marker in ["•", "-", "1.", "2.", "*"]
            ),  # Lists
        ]
        ...
        generic_count = sum(
            1 for phrase in generic_phrases if phrase in response_text.lower()
        )
        ...
        weights = [
            ...
            0.15,
            0.20,
        ]  # Citation and relevance weighted higher
        quality_score = sum(
...
async def get_rag_system_health() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            doc_stats = (
                client.table("documents").select("status", count="exact").execute()
            )
            chunk_stats = (
                client.table("document_chunks").select("id", count="exact").execute()
            )

            health_status["database"]["status"] = "healthy"
            health_status["database"]["total_documents"] = (
                doc_stats.count
                if hasattr(doc_stats, "count")
                else len(doc_stats.data or [])
            )
            health_status["database"]["total_chunks"] = (
                chunk_stats.count
                if hasattr(chunk_stats, "count")
                else len(chunk_stats.data or [])
            )

            # Count completed documents
            if doc_stats.data:
                completed_count = sum(
                    1 for doc in doc_stats.data if doc.get("status") == "completed"
                )
        ...
        if unhealthy_services > 0:
            health_status["overall_status"] = (
                "unhealthy" if unhealthy_services > 1 else "degraded"
            )
