...
class RAGConfig:
    ...
    def validate(self) -> List[str]:
        ...
        if self.max_chunks_per_query <= 0:
            ...
            errors.append("RAG_MAX_CHUNKS_PER_QUERY should not exceed 20 for performance reasons")
...
class FileConfig:
    ...
    allowed_file_extensions: List[str] = field(default_factory=lambda: [".pdf", ".doc", ".docx", ".txt"])
    ...
    def load_from_environment(self) -> SystemConfig:
        ...
        try:
            ...
            ai_config = AIConfig(
                ...
                embedding_model=os.getenv("RAG_EMBEDDING_MODEL", "models/embedding-001"),
                ...
                retry_backoff_factor=float(os.getenv("RAG_RETRY_BACKOFF_FACTOR", "2.0")),
            ...
            allowed_extensions = os.getenv("ALLOWED_FILE_EXTENSIONS", ".pdf,.doc,.docx,.txt")
            file_config = FileConfig(
                max_file_size_mb=int(os.getenv("MAX_FILE_SIZE_MB", "50")),
                allowed_file_extensions=[ext.strip() for ext in allowed_extensions.split(",")]
            )
            ...
            logger.info("Configuration loaded and validated successfully from environment variables")
            return system_config
            
        except (ValueError, TypeError) as e:
            error_message = f"Failed to parse configuration from environment variables: {str(e)}"
            logger.error(error_message)
...
def validate_startup_configuration() -> None:
    ...
    try:
        ...
        logger.info(f"Configuration loaded: {summary['config']['application']['app_name']} v{summary['config']['application']['app_version']}")
        ...
        if config.security.jwt_secret_key == "your-super-secret-jwt-key-change-this-in-production":
            logger.warning("Default JWT_SECRET_KEY detected - change this in production")
        
    except ConfigurationError:
        raise
    except Exception as e:
        error_message = f"Unexpected error during startup configuration validation: {str(e)}"
        logger.error(error_message)
        raise ConfigurationError(error_message)